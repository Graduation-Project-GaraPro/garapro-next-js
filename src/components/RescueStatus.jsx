...
const RescueStatus = ({ status, estimatedArrival, requestTime, updateStatus, generatePdf }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      <div className="p-4 bg-blue-600 text-white font-semibold">Trạng thái cứu hộ</div>
      
      <div className="p-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            <span className="text-sm text-gray-500">Trạng thái:</span>
            <span className={`ml-2 px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(status)}`}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-sm font-medium text-blue-600">{getRemainingTime()}</span>
          </div>
        </div>
        ...
        <div className="mt-6 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {['dispatched', 'on-the-way', 'arrived', 'in-progress', 'completed'].map((statusOption) => (
              <button
                key={statusOption}
                onClick={() => updateStatus(statusOption)}
                disabled={statusOption === status}
                className={`py-2 px-3 rounded-md text-sm transition-colors ${statusOption === status
