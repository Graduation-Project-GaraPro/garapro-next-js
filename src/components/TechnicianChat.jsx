...
const TechnicianChat = ({ messages = [], isOpen, onClose = () => {}, technicianName, initialMessages = [], accentColor = 'blue', sendMessage = () => {} }) => {
  ...
  const getMessageStyle = (sender) => {
    switch (sender) {
      ...
      case 'system':
        return 'bg-yellow-100 text-yellow-800 self-center text-xs';
      default:
  ...
  const normalizeMessage = (msg) => {
    ...
    const sender = senderRaw === 'me' ? 'user' : (senderRaw === 'tech' ? 'technician' : (senderRaw || 'system'));
            ...
            <div key={index} className={`max-w-[80%] rounded-lg p-3 ${getMessageStyle(msg.sender)}`}>
              <div className="flex justify-between items-center mb-1">
                <span className="font-medium text-xs">
                  {msg.sender === 'user' ? 'Bạn' : 
                   msg.sender === 'technician' ? 'Kỹ thuật viên' : 'Hệ thống'}
                </span>
