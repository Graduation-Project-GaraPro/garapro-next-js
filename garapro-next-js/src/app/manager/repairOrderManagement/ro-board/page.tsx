...
} from "@/types/manager/repair-order";
...
export default function BoardPage() {
  ...
  const [editingRepairOrder, setEditingRepairOrder] =
  ...
  const initializePage = async () => {
    ...
    // Load initial data
    await Promise.all([loadRepairOrders(), loadStatuses()]);
  ...
  const handleCreateRepairOrderWrapper = async (jobData: Omit<Job, "id">) => {
    try {
      ...
      console.log(
        "Creating repair order with data:",
        JSON.stringify(createRequest, null, 2)
      );

      // Call the repair order service to create the repair order
      const createdRepairOrder = await repairOrderService.createRepairOrder(
        createRequest
      );
  ...
  const handleUpdateRepairOrder = async (repairOrderData: RepairOrder) => {
    try {
      ...
      setRepairOrders((prev) =>
        prev.map((ro) =>
          ro.repairOrderId === repairOrderData.repairOrderId
            ? repairOrderData
            : ro
        )
      );
  ...
  const handleDeleteRepairOrder = async (repairOrderId: string) => {
    try {
      ...
      setRepairOrders((prev) =>
        prev.filter((ro) => ro.repairOrderId !== repairOrderId)
      );
  ...
  const handleMoveRepairOrder = async (
    repairOrderId: string,
    newStatusId: string
  ) => {
    try {
      // Call the API to update the status
      const success = await repairOrderHubService.updateRepairOrderStatus(
        repairOrderId,
        newStatusId
      );
      ...
      if (success) {
        console.log(
          `Successfully requested move of repair order ${repairOrderId} to status ${newStatusId}`
        );
        // The UI will be updated via SignalR notification, not directly here
      } else {
        console.error(
          `Failed to move repair order ${repairOrderId} to status ${newStatusId}`
        );
  ...
  const handleRepairOrderMoved = (
    repairOrderId: string,
    newStatusId: string,
    updatedCard: RoBoardCardDto
  ) => {
    console.log(
      "Repair order moved via SignalR:",
      repairOrderId,
      newStatusId,
      updatedCard
    );
  ...
  const handleRepairOrderCreated = async (repairOrder: RoBoardCardDto) => {
    const message =
      `🔧 Đơn sửa chữa mới!\n` +
      `Mã đơn: ${repairOrder.repairOrderId || "N/A"}\n` +
      `Khách hàng: ${repairOrder.customerInfo || "N/A"}\n` +
      `Trạng thái: ${repairOrder.statusName || "Mới tạo"}`;

    try {
      // Gửi tin nhắn qua Facebook Messenger
      //  await sendFacebookMessage(message);
      console.log("Đã thông báo qua Facebook Messenger");
    } catch (error) {
      console.error("Không thể gửi thông báo qua Facebook:", error);
    }
  };

  const sendFacebookMessage = async (message: string) => {
    try {
      const response = await apiClient.post("/api/FBMessage", {
        message: message,
      });
