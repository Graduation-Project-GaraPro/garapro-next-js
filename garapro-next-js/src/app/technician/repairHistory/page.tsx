...
export default function RepairHistory() {
  ...
  const [isSorted, setIsSorted] = useState(false); // Thêm trạng thái để kiểm soát sắp xếp
  ...
  const sortByDate = (vehicles: VehicleHistory[]) => {
    return [...vehicles].sort((a, b) => {
      ...
      return latestDateB.getTime() - latestDateA.getTime(); // Gần nhất đến xa nhất
    });
  };

  const sortHistoryByDate = (history: RepairEntry[]) => {
    return [...history].sort((a, b) => {
      return new Date(b.date).getTime() - new Date(a.date).getTime(); // Gần nhất đến xa nhất
    });
  };
  ...
  const filteredVehicles = isSorted
    ? sortByDate(
        vehicleHistories.filter(
          (vehicle) =>
