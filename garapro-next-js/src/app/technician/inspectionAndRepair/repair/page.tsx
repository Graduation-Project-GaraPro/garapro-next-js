...
export default function ConditionInspection() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        const mappedVehicles: Vehicle[] = data.map((item) => {
          // Build vehicle name
  ...
  const handleRepairAction = (jobId: string) => {
                    ...
                    <div className="relative p-6">
                      ...
                      {vehicle.status === "New" ? (
                        <div className="flex items-center justify-between mb-4 gap-4">           
                          <button 
                            onClick={() => handleRepairAction(vehicle.id)}
              ...
              <div className="flex items-center justify-center gap-10 text-center">
                ...
                <div>
                  <div className="text-xl font-bold text-cyan-600">
                    {assignedVehicles.filter(v => v.status === "InProgress").length}
                  </div>
                ...
                <div>
                  <div className="text-xl font-bold text-red-600">
                    {assignedVehicles.filter(v => v.status === "OnHold").length}
                  </div>
